...
const toLastNames = people => {
  // Solution code here...
  let x = people.map( value => `${value.firstName} ${value.lastName}`);
  console.log('this the name', x);
  return x;
};
...
const addValues = (arr) => {
  // Solution code here...
  let x = arr.reduce((accumulator,value,idx) => {
    accumulator = accumulator + value;
    return accumulator;
  }, 0);
  console.log('add value', x);
  return x;
};
...
const addPurchases = (arr) => {
  ...
  let x = arr.reduce((accumulator,value) => {
    // accumulator = value.purchasePrice + value.purchasePrice;
